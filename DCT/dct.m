...
for i = 1 : blockCounter
    ...
    co(:,:,i)=coef;
    TempCoef=zeros(block_size);
    TempCoef2=coef;
    for j=-midFreq:midFreq
        TempCoef=TempCoef+flip(diag(diag(flip(coef),j),j));
    end
    ...
    for k = 1 : block_size
        for l = 1 : block_size
            if (TempCoef(k,l)<0) && (WaterBlock(k,l)==1)
                TempCoef2(k,l)=offset;
            elseif (TempCoef(k,l)>0) && (WaterBlock(k,l)==0)
                TempCoef2(k,l)=-offset;
            end
        end
    end
